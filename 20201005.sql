...
1. 인위적으로 여러개의 행을 만들기
    ...
    EX : '202010' ==> 31
        ...
        TO_CHAR(날짜, '원하는 항목')
        TO_CHAR(해당날짜의 마지막날까지 구하는 함수(TO_DATE('202010', '포맷')), '원하는 항목')
        ...
        해당 날짜의 요일에 따라서 새롭게 만든 7개 컬럼(일~월) 중 날짜를 위치 시킨다.
           SELECT TO_DATE('202010', 'YYYYMM') + LEVEL-1 day,
               TO_CHAR(TO_DATE('202010', 'YYYYMM') + LEVEL-1, 'D' d,
               TO_CHAR(TO_DATE('202010', 'YYYYMM') + LEVEL-1, 'iw' iw,               
        FROM dual
        ...
        SELECT /*day, d, iw, DECODE (d, 1, iw +1, iw)*/
...
DECLARE
    선언부 (생략가능)
            ...
            JAVA랑은 다르게 변수 선언을 블록 어디서나 할 수 없음
BEGIN 
    실행부(생략불가)
    로직-(데이터를 조회해서 변수에 담기, 루프, 조건제어)
EXCEPTION
    예외부(생략가능)
    BEGIN절에서 발생한 예외를 처리하는 부분
END;
/
...
BEGIN
  ...
  /*  Sysout.out.println("v_deptno : " + v_deptno + ", v_dname : " + v_dname); */
...
DECLARE 
    ...
    v_dname dept.dname%TYPE; /* v_dname VARCHAR2(14); */
BEGIN
    SELECT deptno, dname INTO v_deptno, v_dname
    FROM dept
    WHERE deptno = 10;
