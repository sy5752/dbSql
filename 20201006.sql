...
1. %ROWTYPE 행 정보 (컬럼이 복수)를 담을 수 있는 변수(rowtype)
 ...
 2. record type : 행정보, 개발자가 컬럼을 선택하여 타입 생성
 3. table type : 행이 여러개인 값을 저장할 수 있는 변수
 
 SET SERVEROUTPUT ON;
  
  emp테이블에서 7369번 사번의 모든 컬럼정보를 rowtype변수에 저장
...
테이블 타입선언
TYPE 테이블_타입이름 IS TABLE OF 행에 대한 타입 INDEX BY BINARY_INTEGER;
테이블_타입_변수명 테이블_타입이름;
...
BEGIN
    ...
    || ', v_deept(i).dname : ' || v_dept(i).dname);
...
일반 case를 검색 case(java의 if)로 변경
 DECLARE
p NUMBER :=5;
BEGIN
CASE 
...
emp테이블에서  7369사번인 사원의 sal정보를 조회하여
...
1. 7369사번의 sal정보를 조회하여 변수를 담는다
2. 1번에서 담은 변수값을 case표현식을 이용하여 새로운 sal값을 구하고 v_sal변수에 할당
3. 7369 사번의 sal 컬럼을 v_sal값으로 업데이트
