...
[실습 ana4]
SELECT empno, ename, sal, deptno, MIN(sal) 
OVER (PARTITION BY deptno) min_sal
FROM emp;
...
LEAD(sal) OVER (ORDER BY sal DESC, hiredate ASC) lag_sal
FROM emp;
...
이전 n행 값 가져오기
LAG(col [, 건너 뛸 행수 - defalut 1] [, 값이 없을 경우 적용할 기본값])
SELECT empno, ename, hiredate,job, sal,
...
SELECT empno, ename, sal, SUM(sal) OVER (PARTITION BY deptno ORDER BY sal, hiredate ASC
...
SELECT empno, ename, sal, SUM(sal) OVER (ORDER BY sal, hiredate ASC
                         ...
                         SUM(sal) OVER (ORDER BY sal, hiredate ASC
                                       ROW UNBOUNDED PRECEDING)c_sum2                            
FROM emp;
...
1. 요청된 SQL과 동일한 SQL이 이미 실행된 적이 있는지 확인하여
    실행된 적이 있다면 SHARED POOL에 저장된 실행 계획을 재활용한다
    
    1-2 만약 SHARED POOL에 저장된 실행 계획이 없다면 실행계획을 세운다 
        (동일한 SQL이 실행된 적이 없음)
...
LEVEL 2 : REPEATABLE READ (반복적 읽기)
          트랜잭션 안에서 동일한 SELECT 쿼리를 실행해도 트랜잭션의 어떤 위치에서든지
          후행트랜잭션의 변경(UPDATE), 삭제(DELETE)의 영향을 받지 않고 항상 동일한
          실행결과를 조회하는 레벨
          
          오라클에서 공식적으로 지원하지는 않지만 SELECT FOR UPDATE 구문을 통해 효과를 낼 수 있음
