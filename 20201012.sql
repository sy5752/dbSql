...
3. 속성 정의
    ...
    식별자 속성 : entity 구분지어 줄 수 있는 속성(학생- 학번, 사원 - 사번)
    일반 속성 : 업무분석을 통해 정의된 속성
    . 설계 속성 : 업무에 존재하지 않지만 시스템 설계 과정에서 도출된 속성
                (코드 속성 : 상담 엔터티의 상담코드 : 01- 일반상담, 02-상품신청, 03-사용문의)
    파생 속성 : 다른 속성으로부터 계산된 값
              ...
              엔터티 : 게시글, 첨부파일
                     ...
                     ==> 쿼리를 매번 작성하는게 귀찮아서 게시글 엔터티에 첨부파일 개수라는
...
Super Type / sub Type
java의 클래스 상속처럼 공통 분모를 Super Type entity로 설계하고 
sub type마다 필요로 하는 속성은 별도로 관리

ex: 사원의 종류가 내근사원, 설계사, 대리점 3개로 구분되고
3류가 공통으로 갖고 있는 속성과 별도로 갖는 속성이 있을때
공통 속성은 SuperType에
개별 속성은 subType을 통해 관리 할 수 있다

Super Type, Sub Type을 물리 모델로 변경하게 될 경우 선택 방안은 3가지가 존재
1. 하나는 type + 모든 sub Type통함( 생성되는 테이블 수 :1)
하나의 테이블로 모든 속성을 넣는 형태로 단점은 null값을 갖는 컬럼이 많아 질 수 있음
2. super Key, sub type (생성되는 테이블 수 : sub type 개수만큼)
    sub type 별로 테이블 생성 각각 생성되는 테이블수 : 최대 
        
5. 정규화
1~3 방법중 명확한 선택 기준은 없다 - 개인 판단 영역
sub Type별 별도의 속성의 개수가 많지 않으면 1번 고려(유지보수)
sub type 속성과 super type속성의 개수가 비슷할 경우
단 sub type 개수가 적을 경우 : 2번
단 sub type 개수가 많을 경우 : 3번

정규화(normalize) : 데이터 중복을 피하기 위해 구조화하는 프로세스
정규형(normalization) : 정규화의 작업으로 나온 결과물

정규화 순서 : 1정규형 - 2정규형 - 3정규형 -BCNF 정규형 - 4정규형 - 5정규형
        순서가 정해져 있음 3정규형은 1,2, 정규형이 끝난 상태에서 진행
        실무에서는 1~3 정규형까지만 진행, 3정규형 이후 부터는 발생 빈도가 낮아지고, 수고 커짐
...
2 정규형 : 주 식별자에 종속적이지 않은 속성을 분리
        ...
        학생이름은 학생번호식별자 속성에만 종속이 되고 강좌번호와는 관련이 없었음
...
집전화번호, 핸드폰 번호 동일한 데이터 타입으로 관리하는게 가능할 것 같음
...
학생 정보로는 학번, 학생이름, 생년월일, 집주소, 핸드폰 번호, 집 전화번호 관리

교과목으로는 과목명, 수강 대상학년, 교재정보들을 관리하려고 한다

학생은 교과목을 수강하고 수강이력을 관리
    1. 재수강을 허용하지 않는 형태
