...
outerjoin 5
SELECT p.pid, p.pnm, NVL(cu.cnm,'brown') cnm, NVL(c.day, 0) day
 ,NVL(c.cnt,0) cnt
...
SELECT p.pid, p.pnm, : cid cid , c.cnt
FROM cycle c, product p, customer cu;
...
pair wise 개념 : 순서쌍, 두가지 조건을 동시에 만족시키는 데이터를 조회 할때 사용
                AND 논리연산자와 결과 값이 다를 수 있다(아래에서 참고)
서브쿼리 : 복수행, 복수컬럼
SELECT *
FROM emp
...
mgr 7698,7839
deptno 30,10
mgr, deptno => (7698,10) (7698,30) (7839,10) (7839,30)
                         (7698,30) (7839,10) 

SCALAR SUBQUERY : SELELCT 절에 기술된 서브쿼리
                하나의 컬럼
