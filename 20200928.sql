...
pruning branch : 가지치기
SELECT 쿼리 처음 배울 때, 설명해준 SQL구문 실행 순서
...
현재 읽은 행의 deptcd값이 앞으로 읽을 행의 p_depcd컬럼과 같고
앞으로 읽을 행의 dept_cd 컬럼값이 'dept0_01'이 아닐 때 연결하겠다

==> xx회사 밑에는 디자인부, 정보기획부, 정보시스템부 3개의 부가 있는데
    그 중에서 정보기획부를 제외한 2개 부서에 대해서만 연결하겠다
    
행 제한 조건을 WHERE절에 기술 했을 경우
FROM -> START WITH -> WHERE절 순으로 실행 되기 떄문에
1. 계층 탐색을 전부 완료한 후
...
SELECT LEVEL, deptno, LTRIME(SYS_CONNECT_BY_PATH(deptno
FROM dual
CONNECT BY LEVEL <= 10;
...
분석함수, 윈도우 함수
사원중에 가장 많은 급여를 받는 사람의 정보조회(empno, ename,sal)
SELECT MAX(sal)
FROM emp;
...
SQL의 단점 : 행간 연산이 부족 ==> 해당 부분을 보완해주는 것이 분석 함수(윈도우 함수)
...
SELECT empno, ename, sal, deptno, ROUND(AVG(sal) 
OVER (PARTITION BY deptno),2)avg_sal
FROM emp;
