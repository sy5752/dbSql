Exception 예외(자바의 try... catch와 동일) : pl/sql 블럭이 실행되는 동안 발생하는 에러
...
END;
/
하나의 행이 조회되어야 하는 상황에서 여러개의 행이 반환되는 경우(예외 처리가 없을 때)
SET SERVEROUTPUT ON;
DECLARE
...
FUNCTION : 변환 값이 존재하는 PL/SQL 블럭
사반을 받아서 해당 사원의 이름을 반환하는 FUNCTION
...
END;
/
SELECT empno, getEmpName(7369), ename
FROM dual;
...
기본 캐쉬 사이즈가 20개
getDeptNAME(10) => ACCOUNTING 오라클은 10번 인자로 getDeptName을 실행 했을때 ACCOUNTING이라는 
결과값을 기억(캐싱) 이후에 동일한 인자로 함수를 실행하면 함수를 실행하지 않고 캐싱된 값을 반환
...
Trigger(방아쇠) : 설정한 이벤트에 따라 실행할 로직을 담은 객체
    웹 : 로그인 버튼 클릭
        ==> 사용자 아이디 id input 값과, 비밀번호 input 값을 
            파라미터 서버로 전송
    db : 테이블의 데이터가 추가되거나 변경되었을 때 pl/sql블럭을 실행
         users 테이블에 사용자 비밀번호가 변경 되었을 때
         users_history 테이블에 기존에 사용한던 비밀번호를 이력으로 생성
...
users테이블의 pass컬럼이 변경되었을 때 실행할 트리거 생성
...
trg_users_pass트리거에 의해 users_history테이블에 비밀번호 이력이 생긴 것을 볼 수 있다
SELECT *
FROM user_history;
