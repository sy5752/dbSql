...
****알아는 두자(못외워도 찾아서 사용할 수 있으면 된다)
    모델링 툴을 사용하여 설계를 하게 되면 툴에서 설계된 테이블을
    생성하는 구문을 자동으로 만들어준다.
...
제약조건 : 데이터에 이상한 값이 들어가지 않도록 강제하는 설정
        emp 테이블에 empno컬럼에 값이 없는 상태로 들어가는 것을 방지
        emp 테이블에 deptno컬럼의 값이 dept 테이블에 존재하지 않는
        데이터를 업력하는 것을 방지
        emp테이블에 empno 컬럼의 값이 중복되지 않도록 방지
        
제약조건은 4가지가 존재, 그중 한가지의 일부를 별도의 키워드로 제공
ORACLE에서 만들 수 있는 제약조건 5가지
...
제약조건 : deptno컬럼을 PRIMARY KEY 제약조건으로 생성

PRIMART KEY :  UNIQUE(중복된 값이 발생할 수 없다)
                + NOT NULL( NULL값이 들어갈 수 없다)
dept_test 테이블에 데이터가 없는 상태

PRIMARY KEY 제약에 의해 deptno컬럼에는 NULL값이 들어갈 수 없다
...
90번 부서가 이미 존재하는 상태였기 때문에 PRIMARY KEY제약에 의해
정상적으로 입력될 수 없다
Unique constraint (PSY.SYS_C007093) violated
INSERT INTO dept_test VALUES (90, 'ddit', 'daejeon');
...
deptno컬럼의 값은 90으로 같지만 dname 컬럼의 값이 다르기 때문에
PRIMARY KEY(deptno, dname) 설정에 따라 데이터가 입력될 수 있다

복합 컬럼에 대한 제약조건은 컬럼 레벨에서는 설정 불가
테이블 레벨, 혹은 테이블 생성 후 제약조건 추가하는 형태만 가능
INSERT INTO dept_test VALUES(90, 'ddit', 'daejeon');
...
dept_test 테이블의 deptno컬럼에 존재하는 값만 입력이 가능하다

현재는 dept_test 테이블에는 90번 부서만 존재,
그렇기 때문에 emp_test에는 90번 이외의 값 들어갈 수 없음

INSERT INTO emp_test VALUES(9000, 'brown', 90);
INSERT INTO emp_test VALUES(9000, 'sally', 90);
...
INSERT INTO emp_test VALUES(9000, 'brown', 90);
INSERT INTO emp_test VALUES(9000, 'sally', 10);
...
DDL에 의해 자동 커밋, emp테이블에 의해 입력이 확정
CREATE TABLE dept_test(
        empno NUMBER(4),
        ename VARCHAR2(14)
);
...
2. 기존에 존재하는 컬럼의 변경(이름, 데이터 타입)
    **데이터타입의 경우는 이미 데이터가 존재하면 수정이 불가능하다.
    동일한 데이터타입으로 사이즈를 늘리는 경우는 상관 없음
    ==> 설계시 고려를 충분히 하자
...
ALTER TABBLE 테이블명  DROP CONSTRAINT 제약조건명;
...
3. emp_test 테이블의 deptno 컬럼이 dept_test컬럼의 deptno컬럼을 참조하는
    FOREIGN KEY제약조건 추가
